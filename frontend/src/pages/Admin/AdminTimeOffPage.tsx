// frontend/src/pages/Admin/AdminTimeOffPage.tsx
import { useEffect, useMemo, useState } from 'react';
import { useMutation, useQuery, useQueryClient } from '@tanstack/react-query';
import api from '../../api/client';
import { createTimeOff, deleteTimeOff, listTimeOff, type TimeOff } from '../../api/timeoff';
...
export default function AdminTimeOffPage() {
  ...
  const timeoffQ = useQuery({
    ...
    queryFn: () => listTimeOff(selectedBarber ? { barberId: selectedBarber } : {}),
    enabled: true,
    staleTime: 10_000,
  });
  ...
  // Form state
  const [form, setForm] = useState<{ barberId: string; start: string; end: string; reason: string }>(
  ...
  const createMut = useMutation({
    ...
    onMutate: async () => {
      await qc.cancelQueries({ queryKey: ['admin-timeoff', selectedBarber || 'all'] });
    },
    onSuccess: () => {
  ...
  const delMut = useMutation({
    ...
    onMutate: async (id) => {
      await qc.cancelQueries({ queryKey: ['admin-timeoff', selectedBarber || 'all'] });
      const prev = qc.getQueryData<TimeOff[]>(['admin-timeoff', selectedBarber || 'all']);
      if (prev) {
    ...
    onError: (err, _id, ctx) => {
      if (ctx?.prev) qc.setQueryData(['admin-timeoff', selectedBarber || 'all'], ctx.prev);
      toast.error(errorMessage(err));
    },
    onSuccess: () => {
      ...
      <div className="rounded-2xl bg-white border border-neutral-200 p-5 shadow-sm space-y-4">
        ...
        <div className="flex justify-end">
          <button
            ...
            disabled={createMut.isPending || !form.start || !form.end || !form.barberId}
      ...
      <div className="overflow-x-auto rounded-xl border border-neutral-200 bg-white shadow-sm">
        {timeoffQ.isLoading ? (
          ...
          <table className="min-w-full text-sm text-left">
            ...
            <tbody>
              {(timeoffQ.data ?? []).map((t) => (
                <tr key={t._id} className="border-t border-neutral-100 hover:bg-neutral-50 transition">
                  <td className="px-4 py-3">{barberMap.get(t.barberId) ?? t.barberId}</td>
                  <td className="px-4 py-3">{new Date(t.start).toLocaleString()}</td>
                  <td className="px-4 py-3">{new Date(t.end).toLocaleString()}</td>
                  <td className="px-4 py-3 text-neutral-600">{t.reason ?? '—'}</td>
